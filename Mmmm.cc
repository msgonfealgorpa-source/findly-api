...
<div id="auth-overlay" class="auth-overlay">
    <div class="auth-card">
        ...
        <div id="auth-inputs">
            ...
            <button onclick="handleEmailAuth()" class="action-btn buy-now" style="width: 100%; margin-top: 10px;">دخول / إنشاء حساب</button>
        </div>
        ...
        <div class="auth-footer-links" style="margin-top:20px; font-size:0.8rem;">
            <a href="terms.html" target="_blank" style="color:#a78bfa;">شروط الاستخدام</a>
            <span class="dot" style="color:white; margin:0 5px;">•</span>
            <a href="privacy.html" target="_blank" style="color:#a78bfa;">سياسة الخصوصية</a>
        </div>
    </div>
</div>
...
<header>
    ...
    <div class="header-tools">
        ...
        <button class="tool-btn" onclick="logoutUser()" title="تسجيل الخروج">
    ...
    onAuthStateChanged(auth, (user) => {
        ...
        if (user) {
            ...
            if (!sessionStorage.getItem('welcome_shown')) {
                ...
                // alert(`مرحباً بك في Findly Sage، ${name}! ✨`);
...
<script>
  ...
  de: {
    ui: {
      ...
      sub: "Ihr smarter Einkaufsführer",
      energy: "Weise Energie",
      welcome: "Willkommen,",
      p_head: "Profil",
      lbl_name: "Nutzername",
      lbl_budget: "Budget ($)",
  ...
  es: {
    ui: {
      ...
      buy: "Comprar",
      watch: "Vigilar",
      over: "¡Sobre el presupuesto!",
    },
    search: {
      market: "Mercado",
      avg_price: "Precio promedio",
      competitors: "Competidores",
      value: "Valor de la oferta",
      deal_score: "Puntuación",
      forecast: "Pronóstico de precio",
  ...
  const UI = dict[lang]?.ui || dict.en.ui; // للحصول على نص التحذير

  status.style.display = 'block';
  status.innerText = '🔍 ...';
  resultsBox.innerHTML = '';

  // جلب الميزانية المحفوظة
  ...
  try {
    ...
    data.results.forEach(p => {
      ...
      // --- منطق الميزانية (الذي كان مفقوداً) ---
      let budgetClass = '';
      let budgetBadge = '';
      
      // استخراج الرقم من سعر المنتج
      ...
      card.className = `product-card ${budgetClass}`; // إضافة كلاس التنبيه
...
<div class="intel-section">
  ...
  ${
    trust.warnings && trust.warnings.length
      ? `<ul>${trust.warnings.map(w => `<li>${w}</li>`).join('')}</ul>`
      : `<p>✅ ${T.no_risks}</p>`
  }
</div>

<div class="btn-group">
  <a href="${p.link}" target="_blank" class="action-btn buy-now">
    ${T.buy}
  </a>

  <button class="action-btn analyze-btn"
    onclick="openAnalysisModal('${encodeURIComponent(JSON.stringify({
      title: verdict.title,
      reason: verdict.reason,
      average: priceIntel.average,
      score: valueIntel.score
    }))}')">
    ${T.analyze}
  </button>
...
`;
    ...
    // 2. دالة إضافة التنبيهات (المراقبة)
   ...
   function openAnalysisModal(encoded) {
    const data = JSON.parse(decodeURIComponent(encoded));
    const modal = document.getElementById('analysis-modal');
    const body = document.getElementById('analysis-content-body');

    body.innerHTML = `
      <p><b>التقييم:</b> ${data.title || '—'}</p>
      <p style="opacity:.8">${data.reason || '—'}</p>
      <hr>
      <p><b>متوسط السوق:</b> ${data.average}</p>
      <p><b>كفاءة الصفقة:</b> ${data.score}%</p>
    `;

    modal.style.display = 'flex';
}
    
    // 3. دوال التحكم في الواجهة واللغة
    function saveSet() {
    ...
    // تحديث دالة فتح البروفايل لاسترجاع البيانات القديمة
    function openProfile() { 
        document.getElementById('p-modal').style.display = 'flex';
        // استرجاع البيانات المحفوظة وعرضها في الحقول
    ...
    function startVoice() {
        ...
        recognition.onstart = () => {  
            ...
            micBtn.className = "fa-solid fa-microphone-lines mic-icon mic-active";  
        };  
        ...
        recognition.onresult = (event) => {  
            ...
            micBtn.className = "fa-solid fa-microphone-lines mic-icon";  
            runSearch();  
        };  
