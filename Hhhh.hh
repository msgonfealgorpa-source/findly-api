...
<div id="auth-status" style="display:none; color:var(--primary); font-weight:bold; margin-top:30px; font-size: 1.1rem;"></div>
    </div>
</div>
...
<script>
  ...
  tr: {
    ...
    let watchList = [];
try {
    const savedWatch = localStorage.getItem('findly_watch');
    if (savedWatch) {
        watchList = JSON.parse(savedWatch);
    }
} catch (e) {
    console.error("Error loading watch list, resetting...", e);
    localStorage.removeItem('findly_watch'); // حذف البيانات التالفة
    watchList = [];
}
...
data.results.forEach(p => {  
    ...
    // Strategic Mapping (NEW – DO NOT REMOVE ANYTHING ABOVE)
let strategicBadge = '';
