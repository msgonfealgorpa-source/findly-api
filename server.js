/* =========================================
FINDLY SERVER v6.1 - COMPLETE WITH SAGE CORE v4
Ultimate Shopping Intelligence Platform
FIXED: Gemini API + Auth + Search
========================================= */
...
const SAGE_TRANSLATIONS = {
  ...
  fr: { buy_now: "Acheter maintenant", wait: "Attendre", overpriced: "Prix élevé", fair_price: "Prix juste", excellent_deal: "Excellente offre", good_deal: "Bonne offre", insufficient_data: "Données insuffisantes", fake_offer: "Offre potentiellement irréaliste" },
  de: { buy_now: "Jetzt kaufen", wait: "Warten", overpriced: "Überteuert", fair_price: "Fairer Preis", excellent_deal: "Ausgezeichnetes Angebot", good_deal: "Gutes Angebot", insufficient_data: "Unzureichende Daten" },
  es: { buy_now: "Comprar ahora", wait: "Esperar", overpriced: "Precio alto", fair_price: "Precio justo", excellent_deal: "Oferta excelente", good_deal: "Buena oferta", insufficient_data: "Datos insuficientes" },
  tr: { buy_now: "Şimdi Satın Al", wait: "Bekle", overpriced: "Fiyat yüksek", fair_price: "Adil fiyat", excellent_deal: "Mükemmel fırsat", good_deal: "İyi fırsat", insufficient_data: "Yetersiz veri" }
};
...
class PriceIntelligence {
    static analyze(product, marketProducts = [], priceHistory = [], lang = 'ar') {
        ...
        if (marketPrices.length < 3) {
            return {
                priceIntel: { current: currentPrice, average: null, median: null, score: 50, decision: t(lang, 'insufficient_data'), color: '#6b7280', confidence: 30 },
                hasEnoughData: false
            };
        }
        ...
        return {
            ...
            trendIntel: { trend: 'stable', confidence: 50 },
            hasEnoughData: true,
...
async function processChatMessage(message, userId, lang = 'ar') {
    ...
    if (lowerMessage.match(/مرحبا|اهلا|hello|hi|hey/)) intent = 'greeting';
    else if (lowerMessage.match(/ابحث|بحث|search|find|lookup/)) intent = 'search';
    else if (lowerMessage.match(/سعر|price|cost|كم/)) intent = 'price';
    else if (lowerMessage.match(/صفقة|deal|offer|discount|خصم/)) intent = 'deal';
    ...
    if (GEMINI_API_KEY) {
        try {
            ...
            if (aiResult && aiResult.reply) {
                return { reply: aiResult.reply, response: aiResult.reply, intent, language: lang, suggestions: aiResult.suggestions };
            }
        } catch (e) {
    ...
    return { reply, response: reply, intent, language: lang };
}
...
async function trackUserBehavior(userId, eventType, data) {
    ...
    try {
        await UserBehavior.create({ userId, eventType, productId: data.productId, query: data.query, price: data.price, metadata: data.metadata });
        await UserProfile.findOneAndUpdate({ userId }, { $inc: { 'stats.totalSearches': eventType === 'search' ? 1 : 0 }, $set: { lastActive: new Date() } }, { upsert: true });
...
async function getUserHistory(userId) {
    if (!dbConnected || !userId || userId === 'guest') return {};
    try {
        const behaviors = await UserBehavior.find({ userId }).sort({ timestamp: -1 }).limit(100).lean();
...
app.get('/', (req, res) => {
    res.json({
        ...
        version: '6.1.0',
        status: 'running',
        endpoints: { chat: 'POST /chat', search: 'GET /search?q=product', health: 'GET /health' }
    });
});
...
app.post('/chat', async (req, res) => {
    try {
        ...
        console.log('📩 Chat:', { message: message?.substring(0, 50), userId: userId || 'guest' });
        
        if (!message || typeof message !== 'string' || message.trim() === '') {
            return res.json({ reply: '👋 مرحباً! كيف يمكنني مساعدتك؟', response: '👋 مرحباً! كيف يمكنني مساعدتك؟' });
        }
        
        await trackUserBehavior(userId, 'chat', { query: message });
        const result = await processChatMessage(message.trim(), userId, lang);
        
        res.json({ reply: result.reply, response: result.response, intent: result.intent, language: result.language, suggestions: result.suggestions });
    } catch (error) {
        console.error('❌ Chat Error:', error.message);
        res.status(500).json({ reply: '🤔 عذراً، حدث خطأ. حاول مرة أخرى!', response: '🤔 عذراً، حدث خطأ. حاول مرة أخرى!' });
    }
});
...
app.get('/search', async (req, res) => {
    ...
    try {
        ...
        uid = decoded.uid;
        console.log('✅ Auth verified for:', uid);
    } catch (err) {
    ...
    if (dbConnected) {
        ...
        } catch (e) { console.log('Energy check error:', e.message); }
    }
    ...
    try {
        ...
        if (!SEARCHAPI_KEY) {
            return res.status(503).json({ error: 'SEARCHAPI_KEY_NOT_CONFIGURED', message: 'Search API key not set on server' });
        }
        ...
        const results = await Promise.all(baseResults.map(async (item) => {
            ...
            let intelligence = {};
            try {
                intelligence = await SageCore(product, baseResults, priceHistory, userHistory.userEvents, uid, userHistory.profile, lang);
            } catch (e) {
...
app.post('/alerts', async (req, res) => {
    try {
        const { userId, productId, productTitle, targetPrice, currentPrice } = req.body;
        ...
        const alert = await PriceAlert.create({ userId, productId, productTitle, targetPrice, currentPrice });
        console.log('🔔 Price Alert Created:', { userId, targetPrice });
