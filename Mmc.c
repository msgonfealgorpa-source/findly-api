...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; outline: none; }
        ...
        .modal-content {
            background: #0f172a; margin: auto; padding: 25px;
            width: 85%; max-width: 450px; border-radius: 20px; text-align: right;
        ...
        /* كلاسات تجاوز الميزانية - مهمة جداً */
        ...
        .budget-error {
            ...
            padding: 8px 12px; border-radius: 12px; font-size: 0.8rem;
            margin-bottom: 10px; font-weight: bold; border: 1px solid rgba(239, 68, 68, 0.3);
        }
        ...
        /* --- Animations --- */
        @keyframes pulse { 0% { opacity: 1; transform:translateY(-50%) scale(1); } 50% { opacity: 0.5; transform:translateY(-50%) scale(1.1); } 100% { opacity: 1; transform:translateY(-50%) scale(1); } }
        @keyframes slideUp { from { opacity: 0; transform: translateY(20px); } to { opacity: 1; transform: translateY(0); } }
        @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }

      
        .auth-overlay {
            display: none !important; /* سيتم تفعيله عبر JS */
        ...
        .invite-icon {
            font-size: 24px;
            flex-shrink: 0;
            background: radial-gradient(circle at top left, #9f8cff, #6f5cff);
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
        }
...
.affiliate-btn {
  ...
  width: 100%;
  height: 100%;
  background: linear-gradient(135deg,#0f0c29,#1a1443);
  display: none;
  align-items: center;
  justify-content: center;
  z-index: 9999;
...
.progress-fill {
  height: 100%;
  width: 0%;
  background: linear-gradient(90deg,#7b5cff,#9f7bff);
...
<div id="analysisLoader" class="analysis-loader">
  <div class="loader-box">
    <h2 id="analysisText">🔍 جاري تحليل المنتج</h2>
...
<div id="auth-overlay" class="auth-overlay">
    <div class="auth-card">
        <div class="auth-header">
            <h1 class="logo-text">Findly <span>Sage</span></h1>
            <p id="auth-title">سجل دخولك لتبدأ البحث الذكي</p>
        </div>

     
        
        <div id="auth-inputs">
    <input type="email" id="auth-email" placeholder="البريد الإلكتروني" class="s-input">
    <input type="password" id="auth-password" placeholder="كلمة المرور" class="s-input">
    
    <div style="display: flex; gap: 10px; margin-top: 10px;">
        <button onclick="handleLogin()" class="action-btn buy-now" style="flex: 1;">
            تسجيل دخول
        </button>
        
        <button onclick="handleRegister()" class="action-btn ghost" style="flex: 1; border: 1px solid var(--primary);">
            إنشاء حساب
        </button>
    </div>
</div>
        ...
        <a href="privacy.html" style="color:var(--text-muted); margin:0 5px;">سياسة الخصوصية</a>
        <a href="terms.html" style="color:var(--text-muted); margin:0 5px;">شروط الاستخدام</a>
        <a href="about.html" style="color:var(--text-muted); margin:0 5px;">من نحن</a>
    </div>
    ...
    <div class="logo-area">
        <div class="logo">
            Findly <i class="fa-solid fa-wand-magic-sparkles"></i>
        </div>

        <p class="tagline">
            Intelligence Before Purchase
        </p>
    </div>
    ...
    <div class="header-tools">
        ...
        <button class="tool-btn" onclick="toggleMode()"><i class="fa-solid fa-circle-half-stroke"></i></button>
        <div style="position: relative;">
            <button class="tool-btn" onclick="toggleLMenu()"><i class="fa-solid fa-language"></i></button>
            <div id="l-menu" class="lang-menu">
                <div class="lang-item" onclick="setLang('ar')">العربية</div>
                <div class="lang-item" onclick="setLang('en')">English</div>
                <div class="lang-item" onclick="setLang('fr')">Français</div>
                <div class="lang-item" onclick="setLang('de')">Deutsch</div>
                <div class="lang-item" onclick="setLang('es')">Español</div>
                <div class="lang-item" onclick="setLang('tr')">Türkçe</div>
            </div>
        </div>
        <button class="tool-btn" onclick="toggleWatchList()">
        ...
        <button class="tool-btn" onclick="openProfile()"><i class="fa-solid fa-user-gear"></i></button>
        <button class="tool-btn" id="logout-btn" onclick="logoutUser()" title="تسجيل الخروج">
...
<main class="search-area">
    ...
    <div class="search-wrapper">
        <input type="text" id="s-input" class="search-input">
        <i id="mic-btn" class="fa-solid fa-microphone-lines mic-icon" onclick="startVoice()"></i>
    ...
    <div class="invite-card" onclick="copyInvite()">
        ...
        <div class="invite-text" id="inviteText">
            ادعُ أصدقاءك الآن ✨ — ميزات ومكافآت قادمة قريبًا 🎁 — كن من الأوائل 🚀
        </div>
    </div>
    ...
    <div class="cat-grid">
        <div class="cat-card" onclick="quickSearch('ألعاب')"><i class="fa-solid fa-gamepad"></i><span id="cat-game"></span></div>
        <div class="cat-card" onclick="quickSearch('جمال')"><i class="fa-solid fa-sparkles"></i><span id="cat-beauty"></span></div>
        <div class="cat-card" onclick="quickSearch('منزل ذكي')"><i class="fa-solid fa-house-laptop"></i><span id="cat-home"></span></div>
        <div class="cat-card" onclick="quickSearch('هواتف')"><i class="fa-solid fa-mobile-screen-button"></i><span id="cat-c1"></span></div>
        <div class="cat-card" onclick="quickSearch('لابتوب')"><i class="fa-solid fa-laptop"></i><span id="cat-c2"></span></div>
        <div class="cat-card" onclick="quickSearch('ساعات')"><i class="fa-solid fa-stopwatch"></i><span id="cat-c4"></span></div>
    </div>
...
<div class="layout-wrapper">
  ...
  <!-- بطاقة الافليت الجانبية -->
  <div id="affiliateCard" class="affiliate-card" style="display:none;">
      <img id="affImage" src="" />
      <h3 id="affTitle"></h3>
      <p>🔥 عرض مميز مرتبط ببحثك</p>
      <a id="affBtn" target="_blank" class="affiliate-btn">
          🛒 شراء الآن
      </a>
  </div>
</main>
...
<div class="modal" id="w-modal">
    <div class="modal-box">
        ...
        <button onclick="toggleWatchList()" style="background:none; color:var(--text-muted); border:none; margin-top:20px; cursor:pointer; font-size:0.9rem;">إغلاق</button>
    </div>
</div>
...
<div class="modal" id="p-modal">
    <div class="modal-box">
        ...
        <label id="lbl-name" style="font-size:0.85rem; display:block; margin-bottom:5px; opacity:0.7;">اسم المستخدم</label>
        <input type="text" id="in-name" placeholder="أدخل اسمك هنا...">
        <label id="lbl-budget" style="font-size:0.85rem; display:block; margin-top:15px; margin-bottom:5px; opacity:0.7;">الميزانية ($)</label>
        <input type="number" id="in-budget" placeholder="أقصى ميزانية لديك...">
        <button class="action-btn buy-now" onclick="saveSet()" style="width:100%; margin-top:20px;">حفظ التغييرات ✅</button>
        <button onclick="openProfile()" style="background:none; color:var(--text-muted); border:none; margin-top:15px; cursor:pointer; font-size:0.9rem;">إغلاق</button>
    </div>
</div>
...
<div id="analysis-modal" class="analysis-modal">
    <div class="modal-content">
        ...
        <button class="action-btn buy-now" style="margin-top:15px; width:100%" onclick="document.getElementById('analysis-modal').style.display='none'">إغلاق</button>
    </div>
</div>
<script type="module">
    ...
    onAuthStateChanged(auth, (user) => {
        ...
        if (user) {
            // المستخدم مسجل دخول -> إخفاء شاشة القفل
            overlay.style.setProperty('display', 'none', 'important');
            document.body.style.overflow = 'auto';
            localStorage.setItem('findly_uid', user.uid);
            
            if (!sessionStorage.getItem('welcome_shown')) {
                const name = user.displayName || 'أيها الباحث الحكيم';
                sessionStorage.setItem('welcome_shown', 'true');
            }
        } else {
            // المستخدم غير مسجل -> إظهار شاشة القفل
    ...
    window.handleLogin = async () => {
        ...
        try {
            ...
            // لا نحتاج لرسالة نجاح لأن onAuthStateChanged سيقوم بإدخال المستخدم تلقائياً
        } catch (error) {
            let msg = "فشل تسجيل الدخول: ";
            if (error.code === 'auth/user-not-found') msg = "هذا الحساب غير موجود، يرجى إنشاء حساب جديد.";
    ...
    // دالة تسجيل الخروج
    window.logoutUser = () => {
        // التحقق من المتغير lang لضمان عدم حدوث خطأ إذا لم يكن معرفاً
        const currentLang = localStorage.getItem('findly_lang') || 'ar';
        if (confirm(currentLang === 'ar' ? "هل تريد تسجيل الخروج؟" : "Logout?")) {
            signOut(auth).then(() => {
...
<script>
    ...
    // --- Definitions moved to TOP to prevent initialization errors ---
    const INVITE_TEXT = {
  ...
  tr: {
    ...
    // Helper Functions
    function updateInviteText(lang) {
        const el = document.getElementById("inviteText");
        if (!el) return;
        el.textContent = INVITE_TEXT[lang] || INVITE_TEXT.ar;
    }
  ...
  const uid =
    localStorage.getItem("findly_uid") ||
    ("guest_" + Date.now());

  try {
    const res = await fetch("https://findly-api.onrender.com/create-payment", {
      method: "POST",
  ...
  const interval = setInterval(() => {
    if (percent < 90) {
      percent += 10;
      progress.style.width = percent + "%";
      text.innerText = steps[step % steps.length];
      step++;
    }
  }, 800);

  return interval; // مهم جداً
}
    


    function stopAnalysisLoader(interval) {
  const loader = document.getElementById("analysisLoader");
  const progress = document.getElementById("progressFill");
  const text = document.getElementById("analysisText");
  ...
  return; // ⬅️ مهم جداً
    ...
    stopAnalysisLoader(loaderInterval); // ✅ أضف هذا
...
`}
    ...
    async function addToWatch(name, price, link) {
    ...
    function copyInvite() {
        ...
        navigator.clipboard.writeText(link).then(() => {
            ...
            alert('لم يتم نسخ الرابط، انسخه يدويًا:\n' + link);
        });
    }

    function openAnalysisFromBtn(btn) {
        try {
            const data = {
                verdict: JSON.parse(decodeURIComponent(btn.dataset.verdict)),
                priceIntel: JSON.parse(decodeURIComponent(btn.dataset.price)),
                valueIntel: JSON.parse(decodeURIComponent(btn.dataset.value)),
                forecastIntel: JSON.parse(decodeURIComponent(btn.dataset.forecast)),
                trustIntel: JSON.parse(decodeURIComponent(btn.dataset.trust))
            };

            const modal = document.getElementById('analysis-modal');
            const body = document.getElementById('analysis-content-body');

            body.innerHTML = `
            <h3>🧠 تحليل Sage الذكي</h3>
            <hr>
            <p><strong>📊 السوق</strong></p>
            <ul>
                <li>متوسط السعر: $${data.priceIntel.average ?? '—'}</li>
                <li>عدد المنافسين: ${data.valueIntel.competitors ?? 0}</li>
            </ul>
            <p><strong>💎 الصفقة</strong></p>
            <ul>
                <li>درجة الصفقة: ${data.valueIntel.score ?? 0}%</li>
                <li>${data.priceIntel.label ?? ''}</li>
            </ul>
            <p><strong>🔮 التوقع</strong></p>
            <ul>
                <li>الاتجاه: ${data.forecastIntel.trend}</li>
                <li>سعر متوقع: $${data.forecastIntel.expectedPrice}</li>
                <li>ثقة: ${Math.round(data.forecastIntel.confidence * 100)}%</li>
            </ul>
            <p><strong>⚠️ الثقة والمخاطر</strong></p>
            ${data.trustIntel.warnings.length ? `<ul>${data.trustIntel.warnings.map(w => `<li>${w}</li>`).join('')}</ul>` : `<p>لا توجد مخاطر واضحة</p>`}
            `;
    ...
    function toggleLMenu() {
        const m = document.getElementById('l-menu');
        m.style.display = (m.style.display === 'block') ? 'none' : 'block';
    }
    function setLang(l) { lang = l; localStorage.setItem('findly_lang', l); toggleLMenu(); update(); }
    ...
    function quickSearch(q) { document.getElementById('s-input').value = q; runSearch(); }
    function quickFilter(type) { document.getElementById('s-input').value += " " + type; }
    ...
    function update() {
        // Ensure dict exists and has the language, fallback to english if missing
        const d = dict[lang] || dict['en'] || dict['ar']; 
        if (!d) return; // Safety check

        const userName = localStorage.getItem('fn') || (lang === 'ar' ? 'أيها الباحث' : 'Seeker');
        
        // Update Invitation Text
        updateInviteText(lang);

        document.getElementById('txt-energy').innerText = d.ui.energy;
        document.getElementById('greeting').innerText = d.ui.welcome + " " + userName;
        document.getElementById('txt-title').innerText = d.ui.title;
        document.getElementById('txt-sub').innerText = d.ui.sub;
        document.getElementById('txt-deep').innerText = d.ui.deep;
        document.getElementById('tag-cheap').innerText = d.ui.cheap;
        document.getElementById('tag-top').innerText = d.ui.top;
        document.getElementById('tag-new').innerText = d.ui.new;

        document.getElementById('p-head').innerText = d.ui.p_head;
        document.getElementById('lbl-name').innerText = d.ui.lbl_name || "اسم المستخدم";
        document.getElementById('lbl-budget').innerText = d.ui.lbl_budget || "الميزانية ($)";

        document.getElementById('cat-game').innerText = d.ui.game;
        document.getElementById('cat-beauty').innerText = d.ui.beauty;
        document.getElementById('cat-home').innerText = d.ui.home;
        document.getElementById('cat-c1').innerText = d.ui.c1;
        document.getElementById('cat-c2').innerText = d.ui.c2;
        document.getElementById('cat-c4').innerText = d.ui.c4;
    ...
    const safeLink =
        product.intelligence?.finalVerdict?.bestLink ||
        product.link;

    document.getElementById("affImage").src =
        product.thumbnail || product.image;

    document.getElementById("affTitle").innerText =
        product.title;

    const affiliateLink =
        affiliateBase + "?redirect=" + encodeURIComponent(safeLink);

    document.getElementById("affBtn").href = affiliateLink;
    ...
    function startVoice() {
        ...
        if (!SpeechRecognition) {
            alert(lang === 'ar' ? "متصفحك لا يدعم البحث الصوتي" : "Speech recognition not supported in this browser");
            return;
        }
    ...
    window.onclick = function(event) {
        if (event.target.classList.contains('modal') || event.target.classList.contains('analysis-modal')) {
...
const lang = localStorage.getItem("findly_lang") || "ar";
  const T = UPGRADE_SUCCESS_TEXT[lang] || UPGRADE_SUCCESS_TEXT.ar;
  ...
  overlay.innerHTML = `
    ...
    ">
      <h2 style="margin-bottom:12px;">${T.title}</h2>
      <p style="opacity:.9; line-height:1.6;">${T.message}</p>
      <p style="margin-top:10px; opacity:.7;">${T.thanks}</p>
      <button
        style="
          margin-top:18px;
          padding:12px 20px;
...
function handleUpgradeButton() {
  const isPro = localStorage.getItem("findly_pro") === "true";
  ...
  upgradeBtn.style.display = isPro ? "none" : "inline-block";
    ...
    window.onload = () => {
  update(); // الميزات القديمة

  if (location.href.includes("upgrade-success")) {
    showUpgradeSuccess(); // هنا يتم تفعيل PRO
  }

  handleUpgradeButton(); // 🔒 الآن سيختفي زر Upgrade مباشرة
};
