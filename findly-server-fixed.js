...
try {
    ...
    console.log('⚠️ Running in guest mode - authentication disabled');
}
...
console.log('🔥 Firebase:', firebaseInitialized ? '✅ Initialized' : '⚠️ Not Initialized (Guest Mode)');
...
async function processChatMessage(message, userId, lang = 'ar') {
    ...
    if (!message || typeof message !== 'string' || message.trim() === '') {
        ...
        return { 
            reply: defaultResponses.greeting[0], 
            response: defaultResponses.greeting[0], 
            intent: 'general', 
            sentiment: 'neutral', 
            language: lang 
        };
...
async function authenticateUser(req) {
    ...
    // No auth header - return guest user
    ...
    // Firebase not initialized - return guest
...
app.get('/health', (req, res) => {
    res.json({
        ...
        features: ['ai_chat', 'price_intelligence', 'personality_engine', 'merchant_trust', 'fake_deal_detection', 'price_alerts', 'behavior_tracking', 'reviews', 'guest_mode']
    });
});
...
app.get('/', (req, res) => {
    res.json({
        ...
        firebase: firebaseInitialized ? '✅ Initialized' : '⚠️ Guest Mode',
        endpoints: {
            chat: 'POST /chat - AI Shopping Assistant',
            search: 'GET /search?q=product - Smart Product Search (supports guest mode)',
...
app.post('/chat', async (req, res) => {
    try {
        ...
        console.log('📩 Chat:', { message: message?.substring?.(0, 50) || 'empty', userId: userId || 'guest' });
        
        // Track chat if userId provided
...
// Smart Search Endpoint - FIXED
app.get('/search', async (req, res) => {
    const { q, lang = 'ar' } = req.query;
    
    console.log('🔍 Search:', { query: q, lang });
    ...
    // Authenticate user - supports guest mode
    const auth = await authenticateUser(req);
    const uid = auth.uid;
    
    console.log('👤 User:', { uid, isGuest: auth.isGuest });

    // Check energy (only for non-guest users with database)
    ...
    try {
        ...
        if (!SEARCHAPI_KEY) {
            console.error('❌ SEARCHAPI_KEY not configured');
            return res.status(503).json({ 
                success: false,
                error: 'SEARCH_NOT_CONFIGURED', 
                message: 'Search API key not configured. Please add SEARCHAPI_KEY to environment variables.',
                results: [] 
            });
        }
        ...
        // Update energy (only for non-guest users)
...
app.post('/create-payment', async (req, res) => {
    try {
        ...
        if (!NOWPAYMENTS_API_KEY) return res.status(503).json({ 
            success: false,
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`🔥 Firebase: ${firebaseInitialized ? '✅ Initialized' : '⚠️ Guest Mode'}`);
